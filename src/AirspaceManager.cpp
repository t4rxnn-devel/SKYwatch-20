...
AirspaceManager::AirspaceManager() noexcept {
    // Automatically provision a standard Class B Commercial Terminal Core Zone
    AirspaceSector kord_core;
    kord_core.designator_icao = "KORD_CLASS_B";
    kord_core.classification = AirspaceClass::CLASS_B_TERMINAL;
    kord_core.lower_limit_feet = 0.0;
    kord_core.upper_limit_feet = 10000.0;
    // Square polygon bounds representing terminal radius boundaries
    kord_core.horizontal_polygon_nm = { {-10,-10}, {10,-10}, {10,10}, {-10,10} };
    kord_core.current_traffic_load = 0;
    kord_core.max_capacity_threshold = 15;
    provision_airport_airspace(kord_core);

    // Provision a specialized Military Special Use Airspace (SUA Restricted Zone)
    ...
    red_flag_zone.lower_limit_feet = 2000.0;
    red_flag_zone.upper_limit_feet = 60000.0;
    red_flag_zone.horizontal_polygon_nm = { {12,12}, {24,12}, {24,24}, {12,24} };
...
bool AirspaceManager::check_perimeter_penetration(const std::string& callsign, 
    ...
    // Convert current Z coordinate metric from NM back into standard aviation Feet
    double altitude_feet = position.z * 6076.12;

    for (const auto& sector : m_configured_sectors) {
        // Vertical window boundary checks first
        if (altitude_feet < sector.geometry.lower_limit_feet || 
            altitude_feet > sector.geometry.upper_limit_feet) {
            continue;
        }

        // Ray-Casting Point-in-Polygon Algorithm to verify 2D crosshair boundary mappings
        ...
        if (inside_polygon) {
            ...
            if (sector.classification == AirspaceClass::RESTRICTED_SUA) {
                ...
                std::cout << "📡 [ATC INFO]: " << callsign << " handed over to sector " << sector_breached << ".\n";
            }
            return true;
        }
    }
